...
def visualize(mu, sigma, label, color, square=False):
    ...
    linestyle = '--' if square == True else '-'
    plt.plot(x.numpy(), y.numpy(), label=label, color=color, linestyle=linestyle, zorder=2)
    plt.xlabel("x")
    plt.ylabel("p(x)")

def main():
    root = "experiments/expr2"
    ...
    visualize(mu_1, sigma_1, label=r"pdf of $\it{Z}_1=N$(0.01, 0.3)", color="red")
    visualize(mu_1, sigma_1, label=r"squared pdf of $\it{Z}_1$", color="red", square=True)
    visualize(mu_2, sigma_2, label=r"pdf of $\it{Z}_2=N$(0.3, 0.5)", color="blue")
    ...
    plt.tight_layout()
    plt.savefig(f"{root}/inclusion.png", dpi=200)
